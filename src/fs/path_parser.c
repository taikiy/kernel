...
static int
get_drive_by_path(const char** path)
{
...
parse_path_part(struct path_part* last, const char** path)
{
...
void
path_free(struct path_root* root)
{
    if (!root) {
        return;
    }
...
struct path_root*
path_parse(const char* path, const char* current_directory_path)
{
