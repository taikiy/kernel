...
void fat16_to_proper_string(char **out, const char *in, size_t length)
{
...
void fat16_get_full_relative_filename(struct fat_directory_item *item, char *buffer, size_t buffer_size)
{
...
static uint32_t fat16_get_first_fat_sector(struct fat_private_data *private_data)
{
    return private_data->header.primary.reserved_sectors;
}

static int fat16_cluster_to_sector(struct fat_private_data *private_data, int cluster)
{
    return ((cluster - 2) * private_data->header.primary.sectors_per_cluster) + private_data->root_directory.last_sector_position;
}
...
static status_t fat16_read_internal(struct disk *disk, int cluster, int offset, int length, void *buffer)
{
    struct fat_private_data *private_data = (struct fat_private_data *)disk->private_data;
    ...
    int sector_to_read = (fat16_cluster_to_sector(private_data, cluster_to_use) * disk->sector_size) + offset_from_cluster;
...
struct fat_directory *fat16_load_fat_directory(struct disk *disk, struct fat_directory_item *item)
{
    ...
    directory->count = fat16_get_total_items_for_directory(private_data->dir_stream, private_data, directory_sector_pos);
...
struct fat_directory_item *fat16_clone_directory_item(struct fat_directory_item *item, size_t size)
{
...
struct fat_item *fat16_new_fat_item_or_directory_item(struct disk *disk, struct fat_directory_item *item)
{
    ...
    {
        // make a clone of the directory item here. `item` is a pointer that could be freed at any time.
